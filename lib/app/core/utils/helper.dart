...
class Helper {
  ...
  //limitShowList
  static void limitShowList(List list, [int limit = 5]) =>
      (list.length > limit) ? list.removeRange(limit, list.length) : null;
  ...
  static List<T> listGenerateSeparated<T>(int count,
      {required T Function(int index) generator,
      required T Function(int index) separator}) {
    final List<T> list = [];
  ...
  static Future<dynamic> readFileJson(String assets) async =>
      jsonDecode(await rootBundle.loadString(assets));
  ...
  static String generateIdFromDateTimeNow() =>
